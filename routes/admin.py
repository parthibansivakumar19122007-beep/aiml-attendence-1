...
def add_student():
    ...
    if request.method == 'POST':
        ...
        # Validate required
        if not all([name, email, student_id, password, class_id]):
        ...
        try:
            u = User(role='STUDENT', name=name, email=email, is_active=True)
...
def add_faculty():
    ...
    if request.method == 'POST':
        ...
        try:
            u = User(role='FACULTY', name=name, email=email, is_active=True)
            u.set_password(password)
            db.session.add(u)
            db.session.flush()
